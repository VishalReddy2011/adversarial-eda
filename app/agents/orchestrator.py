...
def next_step(df: pd.DataFrame, accepted_count: int, function_calls: int, insight_ledger) -> OrchestratorDecision:
    ...
    base_prompt = (
        "You are the Orchestrator for statistical EDA. Return ONLY JSON. No prose.\n"
        "Output schema: {hypothesis: string, specialist_name: string}.\n"
        "specialist_name must be one of: univariate, bivariate, regression.\n"
        "Selection rules:\n"
        "- univariate: single-variable distribution/normality question.\n"
        "- bivariate: relationship or group-difference between two variables.\n"
        "- regression: predictive relationship with a target variable.\n"
        "Constraints:\n"
        "- Hypothesis must reference existing columns only.\n"
        "- Keep hypothesis specific and testable in one step.\n"
